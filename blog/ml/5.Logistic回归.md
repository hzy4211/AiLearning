...
def gradAscent(dataMatIn, classLabels):
    dataMatrix = mat(dataMatIn)             # 转换为 NumPy 矩阵，mat是numpy下的matrix数据类型，类似pandas中的dataframe数据类型
    ...
    maxCycles = 500     # 迭代次数 
    weights = ones((n,1))  # weights 代表回归系数， 即初始化一个n*1维的，值全为1的权重矩阵，n为特征的个数。
    for k in range(maxCycles):              
        # m*3 的矩阵 * 3*1 的矩阵 ＝ m*1的矩阵
        h = sigmoid(dataMatrix*weights)     # 矩阵乘法
        # labelMat是实际值
        error = (labelMat - h)              # 向量相减，labelMat也是列向量
        # 0.001* (3*m)*(m*1) 表示在每一个列上的一个误差情况，最后得出 x1,x2,xn的系数的偏移量
        weights = weights + alpha * dataMatrix.transpose() * error # 矩阵乘法，最后得到回归系数。逻辑斯蒂回归是采用极大似然估计，因此此处是加法，使用加法因为是梯度上升
...
def plotBestFit(dataArr, labelMat, weights):
    ...
    ax.scatter(xcord1, ycord1, s=30, c='red', marker='s') #正确分类的散点图
    ax.scatter(xcord2, ycord2, s=30, c='green') #错误分类的散点图
    
    #现在我们开始画分割平面w0+w1*x+w2*y=0，初始化x值
...
def stocGradAscent0(dataMatrix, classLabels):
    ...
    # n*1的矩阵
    # 函数ones创建一个全1的数组
    weights = ones(n)   # 初始化长度为n的一维数组，元素全部为 1
    for i in range(m):
...
第一处改进为 alpha 的值。alpha 在每次迭代的时候都会调整，这会缓解上面波动图的数据波动或者高频波动。另外，虽然 alpha 会随着迭代次数不断减少，但永远不会减小到 0，因为我们在计算公式中添加了一个常数项。
...
def stocGradAscent0(dataMatrix, classLabels):
    ...
    # n*1的矩阵
    # 函数ones创建一个全1的数组
    weights = ones(n)   # 初始化长度为n（列数，也是特征数）的数组，元素全部为 1
    for i in range(m):
        # sum(dataMatrix[i]*weights)为了求 f(x)的值， f(x)=a1*x1+b2*x2+..+nn*xn,此处求出的 h 是一个具体的数值，而不是一个矩阵
        h = sigmoid(sum(dataMatrix[i]*weights))
        # print 'dataMatrix[i]===', dataMatrix[i]
...
def stocGradAscent1(dataMatrix, classLabels, numIter=150):
    m,n = shape(dataMatrix)
    weights = ones(n)   # 创建与列数（特征数）相同的矩阵的一维数组，所有的元素都是1
    # 随机梯度, 循环150,观察是否收敛
    for j in range(numIter):
        # [0, 1, 2 .. m-1]
        dataIndex = range(m)
